...
if __name__ == "__main__":
    ...
    matchdatapath = r"C:\Users\CHAITANYA\Documents\IPL\ipl\Data\matchdata.csv"
    matchsummarypath = r"C:\Users\CHAITANYA\Documents\IPL\ipl\Data\matchsummary.csv"
    ...
    # Options include 'rf','xgb','catboost','movingaverage', 'ensemble'
    modelname = 'catboost'
    ...
    receiver_email = "madhavgoswami93@gmail.com," + "sainiabhi7734@gmail.com," + "rapidnehal@gmail.com," + \
        "sandeepch@zeta.tech," + "mandalravi04@gmail.com," + "vikashkumar72741234@gmail.com"
    ...
    if TRAIN_MODEL:
        # Run the function to to get points in the scorecard format
        execute_get_scorecard(
            datapath["matchdatapath"], datapath['matchdatascorecardpath'], pointsconfig)
        execute_featureengg(datapath['matchdatascorecardpath'], datapath['matchsummarypath'],
    ...
    if PREDICT_MODEL:
        ...
        for modelname in modelnamelist:
            ...
            # Run the function to predict the points based on the model
            modelpred_df = execute_model_prediction(
                datapath, predictors, modelname, cat_cols, pred_col + '_' + modelname, usetimeseries=False)
            modelpred_df = modelpred_df[['matchid', 'playername', 'playing_role',
                                         'playing_team', 'playercost', 'total_points', pred_col + '_' + modelname]]
            predcol_list.append(pred_col + '_' + modelname)
            print(f"prediction for model {modelname} is complete")
            finalmodelpred = pd.merge(finalmodelpred, modelpred_df, on=[
                                      'matchid', 'playername', 'playing_role', 'playing_team', 'playercost', 'total_points'], how='left')

        if PREDICT_ENSEMBLE:
            EM = EnsembleModel()
            temp = EM.get_ensemble_model_train(
                finalmodelpred, predcol_list, target_col='total_points', predcol=pred_col, modelpath=r"Data/" + 'ensemble' + "_model.pkl")
            finalmodelpred = pd.merge(
                finalmodelpred, temp, left_index=True, right_index=True, how='left')
        else:
            finalmodelpred[pred_col] = finalmodelpred[pred_col +
                                                      '_' + modelname]
        finalmodelpred.to_csv(datapath['modelresultspath'], index=False)
        # Run the function to only select the predicted playing 11
        execute_team_selection(datapath, constconfig, colconfig)
        # Run the function to estimate rewards if actual playing 11 is available
        execute_rewards_calcualtion(
    ...
    if SELECT_PLAYING_SQUAD:
        ...
        create_pred_dataframe_before_playing_XI(
    ...
    if SELECT_CURRENT_TEAM:
        ...
        for modelname in ['catboost', 'xgb', 'rf', 'movingaverage', 'ensemble']:
            ...
            if modelname == 'ensemble':
                EnsembleModel().get_ensemble_model_pred(
                    datapath, finalteam.copy(), predcol_list, pred_col)
            else:
                execute_model_prediction(
                    datapath, predictors, modelname, cat_cols, pred_col, usetimeseries=False, predpath=True)
            teamtemp = execute_team_selection(
                datapath, constconfig, colconfig).team_points
            print("teamtemp1", teamtemp.iloc[:, 13:15])
            teamtemp.sort_values(by=['matchid', 'pred_selection_true',
                                 'pred_points', 'playername'], inplace=True, ascending=False)
            ...
            teamtemp = teamtemp[['matchid', 'playername', 'playing_role', 'playing_team',
                                 'playercost', 'pred_points' + '_' + modelname, 'pred_selection_true' + '_' + modelname]]
            print("teamtemp2", teamtemp.iloc[:, 4:6])
            predcol_list.append('pred_points' + '_' + modelname)
            if finalteam.shape[0] == 0:
                finalteam = teamtemp
            else:
                finalteam = pd.merge(finalteam, teamtemp, on=[
                                     'matchid', 'playername', 'playing_role', 'playing_team', 'playercost'], how='left')
