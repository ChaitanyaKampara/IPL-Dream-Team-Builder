...
def execute_get_scorecard(matchdatapath, scorecardpath, pointsconfig):
    ...
    ipl_scorecard_points = Dream11Points(
        ipl_scorecard.ipl_merged_scorecard, pointsconfig)
...
def execute_featureengg(matchdatascorecardpath, matchsummarypath, featenggpath, colconfig):
    ...
    for i in rolling_window:
        FeatEng.add_lagging_feat(colconfig['MATCHID'], colconfig['VENUE'],
                                 i, colconfig['TOTALBATPOINTS'], colconfig['TOTALBALLPOINTS'])
        FeatEng.add_lagging_feat(colconfig['MATCHID'], colconfig['PLAYERNAME'], i,
                                 colconfig['ACTUALPOINTS'],  colconfig['BATTINGORDER'], colconfig['TOTALBALLSBOWLED'])
...
def execute_model_train(datapath, modelname, predictors, cat_cols, target_col, usetimeseries=False):
    ...
    if usetimeseries:
        ts_prediction = ModelTrain.get_timeseries_forecast(
            masterdf, target_col, 'playername', 'ts_pred_points')
        masterdf = pd.merge(masterdf, ts_prediction,
    ...
    if modelname == 'movingaverate':
        return
    modeltrain = ModelTrain(masterdf, target_col,
                            predictors, cat_cols, modelname)
    modeltrain.get_normalized_data()
    modeltrain.get_test_train(split_col='year', split_value=[2024])
...
def execute_model_prediction(datapath, predictors, modelname, cat_cols, pred_col, usetimeseries=False, predpath=False):
    ...
    modelpkl = joblib.load(datapath['modelpath'])

    # Load the encoder using joblib
    enc = joblib.load(datapath['encoderpath'])

    mod_predict = ModelPredict(
        masterdf, enc, modelpkl, modelname, predictors, cat_cols, pred_col)
...
def execute_rewards_calcualtion(datapath, constconfig, colconfig, rewardconfig):
    ...
    ipl_team_rewards = RewardEstimate(
        optimum_team.team_points, matchdata.copy())
    ...
    ipl_team_rewards.total_match_points.to_csv(
        datapath['rewardspath'], index=False)
...
def create_pred_dataframe_before_playing_XI(datapath, colconfig, team1, team2, city, venue, toss_winner):
    ...
    ipl_curr_squad = ipl_curr_squad[ipl_curr_squad['playing_team'].isin([
                                                                        team1, team2])]

    ipl_curr_squad['opposition_team'] = np.where(
        ipl_curr_squad['playing_team'] == team1, team2, team1)
    ipl_curr_squad = ipl_curr_squad[[
        'playername', 'playing_team', 'playing_role', 'opposition_team', 'playercost']]
    ...
    summarydf = pd.DataFrame({"matchid": [MATCHID], "venue": [venue], "city": [city], "team1": [team1], "team2": [team2],
                              "year": [2024], "toss_winner": [toss_winner]})
    matchsummary_pred = pd.concat([matchsummary, summarydf], axis=0)
    matchsummary_pred.to_csv(datapath['predsummarypath'], index=False)
    matchdatafeatures = execute_featureengg(
        datapath['predscorecardpath'], datapath['predsummarypath'], datapath['predfeaturepath'], colconfig)
...
def create_pred_dataframe_after_playing_XI(datapath):
    ...
    print(playing_squad)
    if playing_squad is not None:  # Check if playing_squad is not None
        print("shape of playing squad", playing_squad.shape)
    else:
        print("No playing squad found. Unable to create pred dataframe.")
    ...
    if playing_squad.shape[0] != 0:
        prefeaturedata = prefeaturedata[prefeaturedata['playername'].isin(
            playing_squad['playername'])]
    else:
        matchdatatemp = pd.read_csv(datapath['matchdatascorecardpath'])
        matchdatatemp['playing_team'] = np.where(pd.isnull(
            matchdatatemp['batsmen_innings']), matchdatatemp['bowler_bowlingteam'], matchdatatemp['batsmen_battingteam'])
        matchdatatemp2 = matchdatatemp[(
            matchdatatemp['playing_team'].isin(team_list))]
        ...
        matchdatatemp2 = matchdatatemp2.drop_duplicates(
            subset=['playername', 'playing_team'])
        prefeaturedata = prefeaturedata[prefeaturedata['playername'].isin(
            matchdatatemp2['playername'])]
...
def formatdata(finaloutdf):
    ...
    finaloutdf = finaloutdf[['playername', 'playing_team', 'playing_role', 'playercost', 'pred_points_catboost',
                             'pred_selection_true_catboost', 'pred_points_ensemble', 'pred_selection_true_ensemble']]
    finaloutdf.columns = ['playername', 'teamname', 'playingrole', 'playercost',
                          'model1_points', 'model1_team', 'model2_points', 'model2_team']
    finaloutdf['model1_points'] = finaloutdf['model1_points'].apply(
        lambda x: round(x, 0))
    finaloutdf['model2_points'] = finaloutdf['model2_points'].apply(
        lambda x: round(x, 0))
    return finaloutdf
...
def update_master_data(datapath, pointsconfig, year):
    ...
    if matchdata_ipl20.shape[0] != 0:
        ...
        if matchid_list:
            matchdata_ipl20_sub = matchdata_ipl20[matchdata_ipl20['matchid'].isin(
                matchid_list)]
            matchdata = pd.concat(
                [matchdata, matchdata_ipl20_sub], join='inner', axis=0)
        ...
        if matchid_list:
            matchdata_ipl20_sub = matchdata_ipl20[matchdata_ipl20['matchid'].isin(
                matchid_list)]
            matchdata_ipl20_sub.to_csv(
                datapath['matchdatapathipl20'], index=False)
            sorecard_sub = execute_get_scorecard(
                datapath['matchdatapathipl20'], datapath['matchdatascorecardpathipl20'], pointsconfig)
            master_scorecard = pd.concat(
                [master_scorecard, sorecard_sub], join='inner', axis=0)
            print(master_scorecard.columns)
            master_scorecard.to_csv(
                datapath['matchdatascorecardpath'], index=False)
            print("matchscorecard updated complete")

        matchsummary_ipl20 = pd.read_csv(datapath['matchsummarypathipl20'])
        matchsummary_ipl20 = matchsummary_ipl20[~(
            matchsummary_ipl20['winner'] == "Match Tied/Cancelled/Not yet ended")]
        matchsummary = pd.read_csv(datapath['matchsummarypath'])
        ...
        if matchid_list:
            ...
            matchsum_ipl20_sub = matchsummary_ipl20[matchsummary_ipl20['matchid'].isin(
                matchid_list)]
            print(matchsum_ipl20_sub.columns)
            print(matchsummary.columns)
            matchsummary = pd.concat(
                [matchsummary, matchsum_ipl20_sub], axis=0)
...
def get_team_details(datapath, index=0):
    ...
    # Read the match summary CSV file into a DataFrame
    matchsummary = pd.read_csv(datapath['matchsummarypathipl20'])

    # Convert the 'date' column to datetime objects
    matchsummary['date'] = pd.to_datetime(matchsummary['date'])

    # Find the next match after the current date and time in India
    upcoming_matches = matchsummary[matchsummary['date'] > datetime_india]

    # If there are no upcoming matches, get the last match of the season
    if upcoming_matches.empty:
        last_match = matchsummary.iloc[-1]
        matchid = last_match['matchid']
    else:
        next_match = upcoming_matches.iloc[0]
        matchid = next_match['matchid']

    # Filter the DataFrame to get details of the identified match
    today_match = matchsummary[matchsummary['matchid'] == matchid]
    print(today_match)

    # Extract the team names and venue
